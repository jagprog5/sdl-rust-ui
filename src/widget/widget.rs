...
/// update the widget, handling user input and drawing it to the screen.
/// the texture creator must be created from the canvas.
pub fn update_gui(
    ...
    let position = place(widget, sdl2::rect::FRect::new(0., 0., w as f32, h as f32), aspect_ratio_priority)?;
    ...
    widget.update(widget_event_dup)?;
    widget.draw(widget_event)
}
