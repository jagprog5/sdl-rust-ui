...
use tiny_sdl2_gui::{layout::{horizontal_layout::HorizontalLayout, vertical_layout::{MajorAxisMaxLenPolicy, VerticalLayout}}, util::length::{MaxLenFailPolicy, MinLenFailPolicy, MinLenPolicy}, widget::{debug::Debug, strut::Strut, widget::{draw_gui, update_gui, SDLEvent, Widget}}};
...
fn main() -> std::process::ExitCode {
    ...
    horizontal_layout.elems.push(Box::new(Debug {
    ...
    horizontal_layout.elems.push(Box::new(Strut::shrinkable(20., 0.)));

    horizontal_layout.elems.push(Box::new(VerticalLayout {
        elems: vec![Box::new(Debug {
            min_h: (HEIGHT / 4.).into(),
            max_h: (HEIGHT / 3.).into(),
    ...
    let mut sdl = match example_common::sdl_util::SDLSystems::new("debug widget + size constraint + layout", (WIDTH as u32, HEIGHT as u32)) {
        Ok(v) => v,
        Err(e) => 
        {
    ...
    if RESTRICT_MIN_SIZE {
        ...
        let _ = sdl.canvas.window_mut().set_minimum_size(min.0.0 as u32, min.1.0 as u32);
    }
